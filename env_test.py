...
class air_env(gym.Env):
    def __init__(self):
        ...
        #训练数据
        self.data_train = pd.read_excel('环境对应数据.xlsx',sheet_name='Sheet3')
        #建一个字典来表示动作序号和值的对应关系
    ...
    def step(self, action):
        ...
        temperature_index = action % 5
        humidity_index = action // 5
        #为了方便容易看
    ...
    def reset(self, seed=None, options=None):
        ...
        #随机选择一天的数据
        #测试环境有5天的数据
        self.count = np.random.randint(0,5)
        #随机选择一天的数据
        i = 41*self.count
    ...
    def reward_function3(self):
        #有正有负型
        a1 = 0.5
        if self.state[4] >= 25.5 and self.state[4]<=26.5:
            r1 = 0.5
        elif self.state[4] > 26.5:
            r1 = a1 * (26.5-self.state[4])
        else:
            r1 = a1 * (self.state[4]-25.5)     
        #另外关注重点是能耗，能耗越多意味着越差，也可以给几个超参数，自行修改
        if self.count2 >=40:
            r3 = -self.power+40
        else:
...
if __name__ == "__main__":
    ...
    totol_reward = 0
    for j in range(40):
        time_step,reward,done,_,info = env.step(12)
        totol_reward += reward
        print(reward)
    print(totol_reward)
