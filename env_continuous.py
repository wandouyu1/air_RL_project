...
class air_env(gym.Env):
    ...
    def step(self, action):
        #输入动作为整数，两个动作，用numpy数组表示
        #检查动作是否合格
        assert self.action_space.contains(action)
        #动作1：出水温度,6~15整数，向上取整
        chilled_water_supply_temperature = (action[0]*4.5)+10.5
        #动作2：水泵频率,35~50整数，向上取整
        pump_frequency = action[1]*7.5+42.5
        self.P,T,H = self.calculate(self.state[0],self.state[1],self.state[2],self.state[3],self.state[4],self.state[5],chilled_water_supply_temperature,pump_frequency)
        self.power += self.P 
        self.count2 += 1
        #计算下一时刻状态
    ...
    def reset(self, seed=None, options=None):
        ...
        self.state = np.array([self.batch_data['室外干球温度'].values[0], 
                               ...
                               60.0],dtype=np.float32)    #室内湿度初始值为当天8：30的湿度
        #检查状态是否合格
        assert self.observation_space.contains(self.state)
        self.count2 = 0
        self.power = 0
...
#测试一下
# a,b,c = env.calculate(temperature_outdoor_air_now = 35,
...
if __name__ == '__main__':
    ...
    for i in range(50):
        ...
        totol_reward = 0
        for j in range(40):
            time_step,reward,done,_,info = env.step(np.array([0,0],dtype=np.float32))
            totol_reward += reward
            #print(reward)
        print(totol_reward)
